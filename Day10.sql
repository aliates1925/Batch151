...
select urun_id, musteri_id from mart
where exists (select urun_id from nisan mart.urun_id=nisan.urun_id);
...
select * from memurlar order by maas desc offset 2 limit 3;
...
select isyeri,isim,sehir,maas from calisanlar
...
elect marka_isim, calisan_sayisi, 
